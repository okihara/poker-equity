...
function applyText(){
  ...
  for(let tok of txt.split(/[,\s]+/)){
    ...
    const m=tok.split(':');let nm=m[0];
    nm=nm[0].toUpperCase()+nm[1].toUpperCase()+(nm.length>2?nm[2].toLowerCase():'');
...
async function run(){
  const my=++token;
  if(hero.length<2){$('eqv').textContent='–';$('matchup').innerHTML='ヒーローの2枚を選んでください。';$('chips').innerHTML='';
    $('segW').style.width='0';$('segT').style.width='0';$('segL').style.width='0';clearHeat();return;}
  const raw=[];
